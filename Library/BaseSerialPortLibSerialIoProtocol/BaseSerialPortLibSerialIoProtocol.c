...
STATIC EFI_SERIAL_IO_PROTOCOL  *mSio;
...
{
  if ((gBS != NULL) && (mSio == NULL)) {
    gBS->LocateProtocol (&gEfiSerialIoProtocolGuid, NULL, (VOID **)&mSio);
  }
  ...
  )
{
  ...
  SerialPortInitialize ();
  if (mSio != NULL) {
    Written = NumberOfBytes;
    Status  = mSio->Write (mSio, &Written, Buffer);
    if (EFI_ERROR (Status)) {
      return 0;
    }
  ...
  )
{
  ...
  if (mSio != NULL) {
    Read   = NumberOfBytes;
    Status = mSio->Read (mSio, &Read, Buffer);
    if (EFI_ERROR (Status)) {
      return 0;
    }
  ...
  )
{
  ...
  SerialPortInitialize ();
  if (mSio != NULL) {
    Status = mSio->GetControl (mSio, &Control);
  ...
  )
{
  SerialPortInitialize ();
  if (mSio != NULL) {
    return mSio->SetControl (mSio, Control);
  }
  ...
  )
{
  SerialPortInitialize ();
  if (mSio != NULL) {
    return mSio->GetControl (mSio, Control);
  }
