  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    mRealST->BootServices->FreePool (mCachedST->RuntimeServices);
  ...
  )
{
  ...
  //
  ProcessLibraryConstructorList (ImageHandle, mCachedST);
  ...
  //
  Status = ProcessModuleEntryPointList (ImageHandle, mCachedST);

  //
  // If all of the drivers returned errors, then invoke all of the library destructors
  //
  if (EFI_ERROR (Status)) {
    ProcessLibraryDestructorList (ImageHandle, mCachedST);
    mRealST->BootServices->FreePool (mCachedST->RuntimeServices);
    mRealST->BootServices->FreePool (mCachedST->BootServices);
    mRealST->BootServices->FreePool (mCachedST);
  }
