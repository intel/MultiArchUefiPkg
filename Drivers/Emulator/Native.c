  ...
  )
{
  ...
  DEBUG ((
    DEBUG_ERROR,
    "Unsupported native call 0x%lx from %a PC 0x%lx\n",
    OriginalProgramCounter,
    Cpu->Name,
    ReturnAddress
    ));
  EmulatorDump ();
  ...
  )
{
  ...
  DEBUG ((
    DEBUG_VERBOSE,
    "%a 0x%lx -> %a 0x%lx(%lx, %lx, %lx, %lx, %lx, %lx, %lx, %lx, %lx)\n",
    Cpu->Name,
    Lr,
    WrapperCall ? "wrapper" : "native",
    Func.ProgramCounter,
    X0,
    X1,
    X2,
